...
// Model, which embeds all the types we wnat to be available to our application.
...
func (u *PostgresRepository) GetByEmail(email string) (*User, error) {
	...
	query := `select id, email, first_name, last_name, password, user_active, created_at, updated_at from users where
	email = $1`
...
// ans hash match, we return true; otherwise, we return fasle/
